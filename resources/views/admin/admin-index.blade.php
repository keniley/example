            ...
            columns: [
                { 
                    ...
                    template: function(row) {
                        return '<a href="/admin/admins/' + row.id + '">' + row.id + '</a>';
                    }
                },
                { 
                    column: 'active', 
                    ...
                    template: function(row) {
                        if(row.active) {
                            return '<a href="/admin/admins/' + row.id + '"><i class="fas fa-check text-success"></i></a>';
                        }
                        return '<a href="/admin/admins/' + row.id + '"><i class="fas fa-times text-danger"></i></a>';
                    }
                },
                { 
                    column: 'name', 
                    ...
                    width: 35,
                    template: function(row) {
                        return '<a href="/admin/admins/' + row.id + '">'+row.name+'</a>';
                    } 
                },
                { 
                    column: 'email', 
                    ...
                    width: 30,
                    template: function(row) {
                        return '<a href="/admin/admins/' + row.id + '">'+row.email+'</a>';
                    }
                }
            ],
            endpoint: {
...
@section ('breadcrumb')
    <nav aria-label="breadcrumb">
      <ol class="breadcrumb justify-content-center bg-transparent">
        ...
        <li class="breadcrumb-item active" aria-current="page">Seznam administrátorů</li>
      </ol>
    </nav>
@endsection
    ...
    <div class="col col-12">
        <div class="panel bg-white p-4">
            <div class="panel-header border-0 mb-3 pb-3 d-flex justify-content-between">
                <h2 class="p-0 m-0">Administrátoři<small class="d-inline-block ml-2"></small></h2>
                <div class="d-flex">
                    <button data-toggle="modal" data-target="#modal-l" data-content="/admin/modal/new/admin" type="button" class="btn btn-success btn-sm btn-circle materialize waves-circle-light"><i class="icon fas fa-plus"></i></button>
